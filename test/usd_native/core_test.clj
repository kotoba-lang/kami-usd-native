...
(def source (slurp "src/usd_native.kotoba"))
(def expected
  {'adr-value "ADR-2605261800"
   'phase-value "R1.0-path-reservation"
   'kami-name-value "kami-usd-native"
   'status-value "contingent-fallback-pending-viability-gate"
   'triggered-by-value "tinyusdz WASM gate fail (R1.1)"
   'nv-compat-target-0 "omni.usd"
   'nv-compat-target-1 "pxr.Usd"
   'nv-compat-target-2 "pxr.UsdGeom"
   'supported-format-0 "usda"
   'supported-format-1 "usdc"
   'supported-format-2 "usdz"})
...
(deftest reference-preserves-all-reservation-metadata
...
(deftest restricted-javascript-and-typed-wasm-conform
  (let [javascript (compiler/compile-source source :js-kotoba-v1)
        ...
        probe (shell/sh
               ...
               wasm64)]
    (testing "both host targets instantiate and preserve observable values"
...
(deftest production-source-authority
  ...
  (is (= ["src/usd_native.cljc"
          "src/usd_native.kotoba"]
